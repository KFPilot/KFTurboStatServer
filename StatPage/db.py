...
def _get_filtered_session_ids(cur, gametypes, difficulties=None):
    ...
    rows = cur.execute(f"SELECT sessionid FROM sessiontable{where_clause}", params).fetchall()
...
def get_difficulties():
    ...
    if "difficulty" in cols:
        rows = cur.execute("SELECT DISTINCT difficulty FROM sessiontable WHERE difficulty IS NOT NULL").fetchall()
...
def get_aggregate_overview(gametypes=None, difficulties=None):
    ...
    if (gametypes is not None and not gametypes) or (difficulties is not None and not difficulties):
        result = {"total_sessions": 0, "total_players": 0, "status_counts": {}, "top_maps": [], "recent_sessions": []}
    ...
    if status_counts.get("InProgress", 0) > 0:
        ip_where = where_clause + (" AND status='InProgress'" if where_clause else " WHERE status='InProgress'")
        ip_sessions = cur.execute(
            f"SELECT sessionid FROM sessiontable{ip_where}",
            params
        ).fetchall()
        empty_count = 0
    ...
    if "Ended" in status_counts:
        status_counts["Abort"] = status_counts.get("Abort", 0) + status_counts.pop("Ended")
