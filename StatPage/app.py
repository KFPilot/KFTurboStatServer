...
def inject_filter_context():
    ...
    parts += (
        [f"difficulties={d}" for d in selected_diff]
        if selected_diff
        else ["difficulties="]
    )
    return {
...
@app.route("/")
def index():
    overview = db.get_aggregate_overview(
        _get_gametypes_filter(), _get_difficulties_filter()
    )
...
def player_detail(playertableid):
    ...
    data = db.get_player_detail(
        playertableid, _get_gametypes_filter(), _get_difficulties_filter()
    )
    if not data:
        abort(404)
